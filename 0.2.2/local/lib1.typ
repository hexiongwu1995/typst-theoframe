...
#let theoframe(name: [], framename: [], kind: "", color: none, it) = figure(
  rect(
    ...
    #stack(dir: ttb, spacing: 0em)[
      #rect(width: 100%, inset: 1em, outset: 0em, fill: color.lighten(80%).transparentize(70%))[
        #align(left)[#text(fill: color, weight: 700)[#framename #context (
              counter(heading.where(level: 1)).display("1.") + counter(figure.where(kind: kind)).display("a")
            ) #h(1em) ] #text(weight: 500)[#name]]
      ]
  ...
  numbering: it => context {
    let counter-content = (counter(heading.where(level: 1)).display("1.") + numbering("a", it))
    counter-content
  },
  outlined: true,
  caption: none,
...
#let theocolor(name: [], framename: [], kind: "", color: none, sym, it) = figure(
  rect(
    ...
    #align(left)[
      #text(fill: color, weight: 700)[#framename #context (
          counter(heading.where(level: 1)).display("1.") + counter(figure.where(kind: kind)).display("a")
        ) #h(1em) ]
...
#let reset(doc) = {
  ...
  show outline.entry: it => if (it.element.func() != figure) { return it } else if (
    ...
    link(
      it.element.location(),
      it.indented(theo-numbering(it.element), it.inner()),
    )
  }
